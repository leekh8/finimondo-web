...
export class GameState {
  ...
  // ─────────────────────────────────────────────
  //  영속화 (Durable Object 최면 대비)
  // ─────────────────────────────────────────────
  /**
   * 상태는 전부 평범한 데이터(배열·객체·원시값)이고 메서드는 프로토타입에 있으므로
   * JSON.stringify로 그대로 실리고, 프로토타입만 다시 씌우면 복원된다.
   * 새 필드를 추가할 때 여기 손댈 필요는 없지만, Map/Set/함수를 상태에 넣으면 깨진다.
   */
  static fromJSON(obj) {
    return Object.assign(Object.create(GameState.prototype), obj);
  }
